...
def get_data(type='categorical', feature_sets=['oneformer', 'semantic', 'driving']):
    # Load the data
    ...
    if type == 'categorical':
        ...
        return np.array(X_train), np.array(X_test), \
            np.array(y_train).reshape([-1, 10]), np.array(y_test).reshape([-1, 10])
    else:
        return np.array(X_train), np.array(X_test), \
            np.array(y_train).reshape([-1, 1]), np.array(y_test).reshape([-1, 1])


def get_data_for_encoder(feature_sets=['oneformer', 'semantic', 'driving']):
    # Load the data
    data = pd.read_csv('../../0_data/1_intermediate_ML/all_features_one_hot_data.csv')
    # drop na
    data = data.dropna()
...
def get_data_mturk(type='categorical', feature_sets=['oneformer', 'semantic', 'driving']):
    # Load the data
    data = pd.read_csv('../../0_data/1_intermediate_ML/all_features_one_hot_data_mturk.csv')
    # drop na
    data = data.dropna()
    # Define the features and target
    X = data.drop(['demanding_level', 'image_id', 'epoch_index', 'frame', 'complex_index_mturk'], axis=1)
    y = data['complex_index_mturk']
    print(X.columns)
    features = {
        'oneformer': ['road', 'sidewalk', 'pole', 'traffic sign', 'vegetation', 'terrain',
            'sky', 'car', 'traffic light', 'building', 'wall', 'fence', 'truck',
...
def get_data_for_encoder_mturk(feature_sets=['oneformer', 'semantic', 'driving']):
    # Load the data
    data = pd.read_csv('../../0_data/1_intermediate_ML/all_features_one_hot_data_mturk.csv')
    # drop na
    data = data.dropna()
    # Define the features and target
    X = data.drop(['demanding_level', 'image_id', 'epoch_index', 'frame', 'complex_index_mturk'], axis=1)
    y = data[['complex_index_mturk', 'image_id']]
    features = {
        'oneformer': ['road', 'sidewalk', 'pole', 'traffic sign', 'vegetation', 'terrain',
            'sky', 'car', 'traffic light', 'building', 'wall', 'fence', 'truck',
            'bicycle', 'person', 'train', 'bus', 'rider', 'motorcycle',
...
def main():
    ...
    transformed_x.to_csv('../../0_data/1_intermediate_ML/complexity_infused_data_%i_categorical_demanding_all.csv' % N_HIDDEN, index=False)
