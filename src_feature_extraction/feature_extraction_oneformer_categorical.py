...
def get_data(feature_sets=['oneformer']):
    # Load the data
...
def get_data_for_encoder(feature_sets=['oneformer']):
    # Load the data
    data = pd.read_csv('../../0_data/1_intermediate_ML/all_features_one_hot_data.csv')
    # drop na
    data = data.dropna()
...
X_train, X_test, y_train, y_test = get_data(['oneformer'])
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=N_EPOCHS, eta_min=0.0005)
...
x, y = get_data_for_encoder(['oneformer'])
...
transformed_x.to_csv('../../0_data/1_intermediate_ML/complexity_infused_data_%i_categorical_demanding_oneformer.csv' % N_HIDDEN, index=False)