...
def get_data(file, feature):
    ...
    if 'all' in file:
        base = features['oneformer'] + features['driving'] + features['semantic']
    elif 'oneformerDriving' in file:
        base = features['oneformer'] + features['driving']
    else:
        base = features['oneformer']

    # Define the features and target
    X = data.drop(['crash_likelihood', 'image_id'], axis=1)
    if feature == 'base_comp':
        X = X[features['complexity'] + base]
    elif feature == 'base':
        X = X[base]
    elif feature == 'comp':
        X = X[features['complexity']]
    elif feature == 'base_index':
        X = X[base + ['demanding_level']]
    else:
        raise ValueError('Please specify the feature argument as base, comp, base_comp, or base_index')
...
def main():
    X_train, X_test, y_train, y_test = get_data('selected_features_one_hot_complexity_32_continuous_demanding_all_crash.csv', args.feature)
    ...
    N_EPOCHS = 2000
    criterion = CrossEntropyLoss()  # Example loss function for classification
    if args.lr == '0.001':
    ...
    plt.legend()
    plt.savefig('modeling_classification_crash_nn_32_continuous_on_all.png')
