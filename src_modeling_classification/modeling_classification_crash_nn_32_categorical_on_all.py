...
def get_data():
    # Load the data
    data = pd.read_csv('../../0_data/1_intermediate_ML/selected_features_one_hot_complexity_32_categorical_demanding_all_crash.csv')
    features = {
    ...
    if args.feature == 'all':
        X = X[features['complexity'] + features['oneformer'] + features['driving'] + features['semantic']]
    else:
...
def main():
    ...
    plt.legend()
    plt.savefig('modeling_classification_crash_nn_32_categorical_on_all.png')
